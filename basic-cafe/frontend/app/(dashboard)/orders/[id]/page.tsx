...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!order) return <div className="text-zinc-400">Order not found</div>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">{order.table ? `Table ${order.table.number}` : "Takeout"}</h1>
          <p className="text-sm text-zinc-400 font-mono">{order.id}</p>
          <p className="text-xs text-zinc-500 mt-0.5">{formatDate(order.openedAt)}</p>
        </div>
      ...
      <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
        ...
        <div className="divide-y divide-zinc-800">
          ...
          {order.items.length === 0 && <p className="py-4 text-center text-sm text-zinc-500">No items yet</p>}
        </div>
        <Separator className="my-3 bg-zinc-800" />
        <div className="flex justify-between text-lg font-bold"><span className="text-zinc-300">Total</span><span className="text-white">{formatCurrency(total)}</span></div>
      </div>
      <div className="flex gap-3">
        {nextStatus && <Button onClick={() => changeStatus(nextStatus)} disabled={statusPending} className="flex-1 bg-indigo-600 hover:bg-indigo-700">{statusActionLabel[order.status]}</Button>}
        {order.status === "delivered" && <Link href={`/checkout/${order.id}`} className="flex-1"><Button className="w-full gap-2 bg-emerald-600 hover:bg-emerald-700"><CreditCard className="h-4 w-4" /> Proceed to Checkout</Button></Link>}
      </div>
